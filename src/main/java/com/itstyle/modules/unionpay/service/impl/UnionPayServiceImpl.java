...
public class UnionPayServiceImpl implements IUnionPayService {
    ...
    public String unionPay(Product product) {
        ...
        //商户订单号，8-40位数字字母，不能含"-"或"_"
        requestData.put("orderId",product.getOutTradeNo());
        //订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
        requestData.put("txnTime",UnionConfig.getCurrentTime());
        //交易币种（境内商户一般是156->人民币）
        requestData.put("currencyCode","156");
        //交易金额，单位分，不要带有小数点
        requestData.put("txnAmt", CommonUtils.subZeroAndDot(product.getTotalFee()));
        /*
            这里组织穿透数据 业务以及交易类型(使用json数据报错)
            请求方保留域，如需使用请启用即可；透传字段（可以实现商户自定义参数追踪）本交易的后台通知,对本交易的交易状态
            查询交易、对账文件中均会原样返回，商户可以按需上传，长度为1-1024个字节
         */
        requestData.put("reqReserved","自定义参数");
        /*
            前台通知地址（需设置为外网能访问http、https均可），支付成功后的页面，点击“返回商户”按钮的时候将异步通知
            报文post到该地址
            如果需要实现过几秒中自动跳转回商户权限页面，需联系银联业务中请开通自动返回商户权限
            异步通知参数详见open.unionpay.com帮助中心 下载 产品接口规范 网关支付产品接口规范 消费交易 商户通知
         */
        //requestData.put("frontUrl",UnionConfig.frontUrl);
        /*
            后台通知地址(设置为外网可访问的http、https均可)，支付成功后银联会自动将异步通知报文post到商户上送的该地址，失败的
            交易银联不会发送后台通知
            后台通知参数详见open.unionpay.com 帮助中心 下载 产品接口规范 网关支付产品接口规范 消费交易 商户通知
            注意：
                1、需要设置为外网可以访问，否则收不到通知
                2、http、https均可
                3、收单后台通知后需要10秒内返回http200或302状态码
                4、如果银联通知服务器发送通知后10秒内未收到返回状态码或者应答码非http200，那么银联会隔一段时间再次发送，总共发送五次，每次间隔时间为0、1、2、4分钟
        ...
        Map<String,String> submitFormData = AcpService.sign(requestData,UnionConfig.encoding_UTF8);
        //获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acpsdk.frontTransUrl
        String requestFrontUrl = SDKConfig.getConfig().getFrontRequestUrl();
        //生成自动跳转的Html表单
        String form = AcpService.createAutoFormHtml(requestFrontUrl,submitFormData,UnionConfig.encoding_UTF8);
        logger.info("打印请求HTML，此为请求报文，为联调排查问题的依据：{}",form);
        //将生成的html写到浏览器中完成自动跳转打开银联支付页面；这里调用singData之后，将html写到浏览器跳转到银联页面之前均不能对html中的表单项的名称和值进行修改，如果修改会导致验签失败
        //resp.getWriter().write(form);
        return form;
    }

    @Override
    public String validate(Map<String, String> validateData, String encoding) {
    ...
    public void fileTransfer() {
        ...
        /**
         * 银联全渠道系统，产品参数，除了encoding自行选择之外，其他不需要修改
         */
        //版本号，全渠道默认值
        data.put("version",UnionConfig.version);
        //字符集编码 可以使用UTF-8，GBK两种形式
        data.put("encoding",UnionConfig.encoding_UTF8);
        //签名方法
        data.put("signtMethod","01");
        //交易类型 76-对账文件下载
        data.put("txnType","76");
        //交易子类型01-对账文件下载
        ...
        //商户代码，请替换正式商户号测试，如使用的是自助化平台注册的777开头商户号，该商户号是没有权限测文件下载接口的
        //请使用测试参数里写的文件下载的商户号和日期测。如需777商户号的真是交易的对账文件，请使用自助化平台下载文件
        ...
        //报文中certId，sinature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可
        String url = SDKConfig.getConfig().getFileTransUrl();
        // 获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acp_sdk.fileTransUrl
